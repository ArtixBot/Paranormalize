...
public class BuffProtection : AbstractStatusEffect, IEventHandler<CombatEventDamageTaken>, IEventHandler<CombatEventRoundEnd>{

    public static string id = "PROTECTION";
    private static Localization.EffectStrings strings = Localization.LocalizationLibrary.Instance.GetEffectStrings(id);

    public BuffProtection() : base(
        id,
        strings,
    ...
    public void HandleEvent(CombatEventDamageTaken data){
        if (data.target == this.OWNER && !data.isPoiseDamage){
            Logging.Log($"{this.OWNER.CHAR_NAME} has Protection, reducing incoming damage from {data.damageTaken} to {data.damageTaken - this.STACKS}.", Logging.LogLevel.INFO);
